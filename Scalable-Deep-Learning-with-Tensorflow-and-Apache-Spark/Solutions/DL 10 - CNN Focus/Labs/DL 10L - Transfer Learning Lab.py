...
train_sample_normal = spark.read.format("binaryFile").load(f"{datasets_dir}/chest-xray/train/normal/IM-0115-0001.jpeg").withColumn("label", lit(0))
train_sample_pneumonia = (spark.read.format("binaryFile")
                          .load([f"{datasets_dir}/chest-xray/train/pneumonia/person1000_bacteria_2931.jpeg",
                                 f"{datasets_dir}/chest-xray/train/pneumonia/person1000_virus_1681.jpeg"])
...
train_generator = datagen.flow_from_directory(directory=f"{datasets_dir}/chest-xray/train/".replace("dbfs:/", "/dbfs/"), 
                                              class_mode="binary", 
...
test_generator = datagen.flow_from_directory(directory=f"{datasets_dir}/chest-xray/test/".replace("dbfs:/", "/dbfs/"), 
                                             class_mode="binary", 
                                             classes=["normal", "pneumonia"],
                                             batch_size=batch_size,
                                             target_size=(img_height, img_width))
