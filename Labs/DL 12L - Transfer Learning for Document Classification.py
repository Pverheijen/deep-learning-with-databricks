...
df = (spark.read
      ...
      .option("sep", ";")
      .format("csv")
      .load(f"{datasets_dir}/news/labelled_newscatcher_dataset.csv"))
...
encoder = (UniversalSentenceEncoder.pretrained()
          .setInputCols(<FILL IN>)
          .setOutputCol(<FILL IN>)
          )

text_classifier = (ClassifierDLApproach()
                   .setInputCols(<FILL IN>)
                   .setOutputCol(<FILL IN>)
                   .setLabelColumn(<FILL IN>)
                   .setMaxEpochs(<FILL IN>)
                  )

classifier_pipeline = Pipeline(stages=[<FILL IN>])
...
print(classification_report(<FILL_IN>))
